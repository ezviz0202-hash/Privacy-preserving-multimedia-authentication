...
def run_demo():
    ...
    print(f"[2/4] Initialising pipeline (epsilon={EPSILON}) ...")
...
def _plot_results(labels, similarities, matched, thresholds):
    ...
    max_thr = max(thresholds) if thresholds else 1.0
    max_val = max(max_sim, max_thr)

    # If all similarities are tiny, zoom in so differences are visible.
    if max_sim <= 0.15:
        ax.set_ylim(0, max_val * 1.15 + 0.02)
    else:
        ax.set_ylim(0, max(1.05, max_val * 1.08))

    for bar, sim in zip(bars, similarities):
        ax.text(
            bar.get_x() + bar.get_width() / 2,
            sim + max(0.006, max_val * 0.02),
            f"{sim:.2f}",
            ha="center",
            va="bottom",
            fontsize=7,
    ...
    ax.set_title(
        ...
        "(dashed line = adaptive threshold per content type)",
        fontsize=11,
    )
    ...
    ax.legend(
        handles=[pass_patch, fail_patch, threshold_patch],
        loc="upper right",
        fontsize=9,
        frameon=True,
    )
    ...
    fig.tight_layout()
    fig.savefig("results/similarity_chart.png", dpi=160, bbox_inches="tight")
    plt.close(fig)
